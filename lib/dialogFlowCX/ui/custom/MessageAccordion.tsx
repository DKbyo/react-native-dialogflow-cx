...
const MessageAccordion = ({
  ...
  const initContentHeight = useCallback(
    (evt) => {
      if (!loaded) {
        setLoaded(true);
  ...
  const animatedHeight = collapsed.interpolate({
    inputRange: [0, 1],
    outputRange: [0, height],
  });
  const animatedMargin = collapsed.interpolate({
    inputRange: [0, 1],
    outputRange: [0, 10],
  });
  const animatedSpin = collapsed.interpolate({
    inputRange: [0, 1],
          ...
          <View style={styles.wrapText}>
            ...
            <Animated.Text
              style={[
                ...
                { color },
                loaded
                  ? { height: animatedHeight, marginTop: animatedMargin }
                  : {},
              ]}
              onLayout={initContentHeight}
            >
