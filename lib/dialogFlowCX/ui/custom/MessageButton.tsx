...
> {
  ...
  render(){
    ...
    return(
      <TouchableOpacity style={[styles.touchable,{backgroundColor}]} onPress={this.handlePress} disabled={!currentMessage.button.link && !currentMessage.button.event}>
        <View style={styles.container}>
          {currentMessage.button.icon && (
            <View style={styles.iconWrapper}>
              <Icon style={{fontSize:25}} name={currentMessage.button.icon.replace('_','-')} color={currentMessage.button.color} />
            </View>
          )}          
