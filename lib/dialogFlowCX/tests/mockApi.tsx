...
class DialogFlowAPI {
  ...
  constructor({
    url = "https://dialogflow.cloud.google.com/v1/cx/locations/{location}/integrations/messenger/webhook/{agentId}/sessions/{session}:detectIntent",
    session,
    agent,
    location,
    onSend,
    ...
    this.#url = url
      .replace("{agentId}", agent)
      .replace("{session}", session)
      .replace("{location}", location);
    this.onSend = onSend;
  }
  detectIntentEvent = async (
    event,
    parameters,
  ): Promise<DialogFlowResponse> => {
    if (this.onSend) {
      this.onSend({
        queryInput: { evemt: { event } },
